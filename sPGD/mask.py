...
def _project_topk_binary(mask: torch.Tensor, k: int) -> torch.Tensor:
    b, c, h, w = mask.shape
    ...
    return torch.zeros_like(flat).scatter_(1, idx, 1).view(b, c, h, w)
...
class MaskingA(torch.autograd.Function):
    ...
    def forward(ctx, x, mask, k):
        ...
        mask_proj = _project_topk_binary(mask, k)
        # stochastic sampling
